...
s_version="0.89"
verbose=1
# verbose=0 only normal stdout
# verbose=1 save log into file
...
function ftp_login {
	if ((verbose)); then
		echo "debug 8 -t -o "$scriptdir/run/$user.lftpdebug"" >> "$ftplogin_file"
	fi
		#Timeout settings
	...
	if  [[ $transferetype =~ "ftp" ]] || [[ $transferetype =~ "fxp" ]]; then
...
function ftp_transfere {
	...
	if [[ $transferetype == "upftp" ]]; then
		...
		fi  # else assume $retry_option == "complete"
...
function logrotate {
	if [[ $test_mode != "true" ]]; then
			...
			if [[ $logrotate == "true" ]]; then
				oldlogfile=$(basename $logfile)
				oldlogname=${oldlogfile%.*}
				oldfileextension=${oldlogfile##*.}
				oldfilepath=$(dirname $logfile)
				oldlogfile=$oldfilepath/$oldlogname.old
...
function loadConfig {
# load config from user config, external config or default config.
...
if [[ $confirm_online == "true" ]] && [[ -e "$scriptdir/dependencies/ftp_online_test.sh" ]]; then
	source "$scriptdir/dependencies/ftp_online_test.sh"
	ftp_test
elif [[ $confirm_online == "true" ]] && [[ ! -e "$scriptdir/dependencies/ftp_online_test.sh" ]]; then
	echo -e "\e[00;33mERROR: Confirm availability is not supported. Couldn't find /dependencies/ftp_online_test.sh\e[00m"
fi


#Check if enough free space on ftp
if [[ "$ftpsizemanagement" == "true" ]] && [[ -e "$scriptdir/dependencies/ftp_size_management.sh" ]]; then
	source "$scriptdir/dependencies/ftp_size_management.sh"
	ftp_sizemanagement check
elif [[ $confirm_online == "true" ]] && [[ ! -e "$scriptdir/dependencies/ftp_size_management.sh" ]]; then
	echo -e "\e[00;33mERROR: FTP sizemanagement not supported. Couldn't find /dependencies/ftp_size_management.sh\e[00m"
fi

#Is largest file too large
if [[ "$split_files" == "true" ]] && [[ "$video_file_only" != "true" ]] && [[ -e "$scriptdir/dependencies/largefile.sh" ]]; then
	source "$scriptdir/dependencies/largefile.sh"
	largefile "$filepath" "exclude_array[@]"
elif [[ "$split_files" == "true" ]] && [[ "$video_file_only" != "true" ]] && [[ ! -e "$scriptdir/dependencies/largefile.sh" ]]; then
	echo -e "\e[00;33mERROR: Split of files is not supported. Couldn't find /dependencies/largefile.sh\e[00m"
fi

# Try to only send videofile
if [[ "$video_file_only" == "true" ]] && [[ "$split_files" != "true" ]] && [[ -e "$scriptdir/plugins/videofile.sh" ]]; then
	source "$scriptdir/plugins/videofile.sh"
	videoFile
elif [[ "$video_file_only" == "true" ]] && [[ "$split_files" != "true" ]] && [[ ! -e "$scriptdir/plugins/videofile.sh" ]]; then
	echo -e "\e[00;33mERROR: Sending videofile only, is not supported. Couldn't find /plugins/videofile.sh\e[00m"
fi

# Try to sort files
if [[ "$sort" == "true" ]] && [[ -e "$scriptdir/dependencies/sorting.sh" ]]; then
	source "$scriptdir/dependencies/sorting.sh"
	sortFiles $category
elif [[ "$sort" == "true" ]] && [[ ! -e "$scriptdir/dependencies/sorting.sh" ]]; then
	echo -e "\e[00;33mERROR: Sorting is not supported. Couldn't find /dependencies/sorting.sh\e[00m"
fi

# Remove item from flexget config
if [[ -n "$feed_name" ]] && [[ -e "$scriptdir/plugins/flexget.sh" ]]; then
	source "$scriptdir/plugins/flexget.sh"
	flexget_feed
elif [[ -n "$feed_name" ]] && [[ ! -e "$scriptdir/plugins/flexget.sh" ]]; then
	echo -e "\e[00;33mERROR: Remove item from Flexget config not supported. Couldn't find /plugins/flexget.sh\e[00m"
fi
...
#Load dependencies
if [[ -e "$scriptdir/dependencies/setup.sh" ]]; then
	source "$scriptdir/dependencies/setup.sh"
else
	echo -e "\e[00;31mError: "$scriptdir/dependencies/setup.sh" is\n needed in order for this program to work\e[00m";
	exit 1
fi
...
do
	case "$1" in
		...
		--path | -p ) if (($# > 1 )); then filepath=$2; else echo -e "\e[00;31mInvalid option for argument '$@'\e[00m"; load_help; show_help; exit 1; fi; shift 2;;
		--path=* ) filepath=${1#--path=}; shift;;
		--feed ) if (($# > 1 )); then feed=$2; else echo -e "\e[00;31mInvalid option for argument '$@'\e[00m"; load_help; show_help; exit 1; fi; shift 2;;
		--feed=* ) feed=${1#--feed=}; shift;;
		--user | -u ) if (($# > 1 )); then user=$2; else echo -e "\e[00;31mInvalid option for argument '$@'\e[00m"; load_help; show_help; exit 1; fi; shift 2;;
		--user=* ) user=${1#--user=}; shift;;
		--exec_post ) if (($# > 1 )); then exec_post=$2; else echo -e "\e[00;31mInvalid option for argument '$@'\e[00m"; load_help; show_help; exit 1; fi; shift 2;;
		--exec_post=* ) exec_post=${1#--exec_post=}; shift;;
		--exec_pre ) if (($# > 1 )); then exec_pre=$2; else echo -e "\e[00;31mInvalid option for argument '$@'\e[00m"; load_help; show_help; exit 1; fi; shift 2;;
		--exec_pre=* ) exec_pre=${1#--exec_pre=}; shift;;
		--delay | -d ) if (($# > 1 )); then date_time=$2; else echo -e "\e[00;31mInvalid option for argument '$@'\e[00m"; load_help; show_help; exit 1; fi; shift 2;;
		--delay=* ) date_time=${1#--delay=}; shift;;
		--force | -f ) force=true; shift;;
		--source=* ) source=${1#--source=}; if [[ -z $source ]]; then load_help; show_help; exit 1; fi; shift;;
		--source | -s ) if (($# > 1 )); then source=$2; if [[ -z $source ]]; then load_help; show_help; exit 1; fi; else echo "Invalid option for argument '$@'"; load_help; show_help; exit 1; fi; shift 2;;
		--wconfig ) option="wconfig"; shift;;
		--category=* ) category=${1#--category=}; shift;;
		--category | -category ) if (($# > 1 )); then cat=$2; else echo -e "\e[00;31mInvalid option for argument '$@'\e[00m"; load_help; show_help; exit 1; fi; shift 2;;		
...
echo
echo -e "\e[00;34mFTP AUTODOWNLOAD SCRIPT - v$s_version\e[00m"
echo
...
case "$option" in
	...
	;;
	"wconfig" ) # Write ou config
		load_help; write_config; exit 0
	;;
	"online" ) # Perform server test
		if [[ -e "$scriptdir/dependencies/ftp_online_test.sh" ]]; then
			source "$scriptdir/dependencies/ftp_online_test.sh"
			ftp_login
			ftp_test
			cleanup session
			...
			exit 0
		echo
			echo -e "\e[00;31mERROR: FTP sizemanagement not supported. Couldn't find /dependencies/ftp_size_management.sh\e[00m"
			exit 1
		fi
	;;
	* ) # main program
