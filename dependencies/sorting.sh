...
function sortFiles {
    ...
    # Check file name patterns and sort accordingly
        ...
        echo "INFO: Sorted to ${complete}"
    elif [[ $orig_name =~ (PAL|NTSC|HDDVD|BluRay|WEB-DL|WEB|BDRip|DVDRip).+(DVDR|XviD|x264|x265|H(264|H265)|H\.(264|265)) ]] && [[ ! $orig_name =~ (S[0-9][0-9]) ]] && [[ ! $orig_name =~ (S[0-9][0-9]E[0-9][0-9]) ]] && [[ ! $orig_name =~ (E[0-9][0-9]) ]] && [[ ! $orig_name =~ (S[0-9][0-9]) ]] && [[ ! $orig_name =~ (\.(episode|ep|e|Part|pt)\.(([0-9][0-9]?)|(I|II|III|IV|V|VI|VII|VIII|IIX|IX|X|XI|XII))\.) ]]  && [[ ! $orig_name =~ ([[:digit:]](x|of)[[:digit:]]) ]]; then
